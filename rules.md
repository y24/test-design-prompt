了解。まずは「rules.md」のドラフトを出すよ。これは“LLMが最初に読む運用手順”として、利用手順書などを材料に「①テスト観点の抽出 → ②業務シナリオ設計」を確実に進めるための共通ルールをまとめたもの。テンプレートや詳細の具体プロンプトは次の2ファイル（`test_viewpoint_identify_guide.md` / `test_scenario_design_guide.md`）側で示す前提だよ。
以下、そのまま`rules.md`として保存して使って。
...
次は `test_viewpoint_identify_guide.md`（観点抽出の具体テンプレート）を用意するね。そこでは Few-shot の具体例と、プロンプト雛形、入力の貼り方、出力のJSON例を詳しく詰める。

...
* **分類必須**: `TestCaseType` を `正常系 | 準正常系 | 異常系 | 非機能` のいずれかで付与。
  * 正常系: 主要フローで想定成功。
  * 準正常系: 想定内エラー/回復可能な分岐（境界・入力揺れ・軽微な障害）。
  * 異常系: 想定外または安全側に倒すべき異常（例: 通信断、整合性崩壊）。
  * 非機能: 性能、セキュリティ、可用性、操作性、監査、アクセシビリティ等。
...
3. **分類付与**: 候補に `TestCaseType` を割当（正常系/準正常系/異常系/非機能）。
...
8. **サマリ**: 観点の偏り（正常系/準正常系/異常系/非機能 の比率）と盲点を要約。
...
- 文書: <名称/版/日付>
- 対象範囲: <機能/画面/ユースケース等>
- 抽出件数: <n>（正常系:<n>, 準正常系:<n>, 異常系:<n>, 非機能:<n>）
- 盲点/留意: <簡潔に>
...
|---|---|---|---|---|---:|---|---|---|---|---|---|---|---|
| VP-001 | <短い名称> | 正常系 | <何を検証するかを一文で> | <章/節/表/図/API/行> | 80 | High | <理由> | <前提> | <データ条件> | <UI/ログ/DB/API> | <任意> | <関連ID> | <仮定> |
| VP-002 | ... | ... | ... | ... | ... | ... | ... | ... | ... | ... | ... | ... | ... |

## 所感
- 偏り: 正常系 が多く 異常系 が手薄。入力検証と通信エラーを追加推奨。
...
| ViewpointID | Title   | TestCaseType      | Description                 | SourceRef | RiskScore | RiskRank | RiskDetail    | Preconditions | DataNeeds          | Observability        | Tags       | Duplicates | Assumptions |
| ----------- | ------- | ------------- | --------------------------- | --------- | --------: | -------- | ------------- | ------------- | ------------------ | -------------------- | ---------- | ---------- | ----------- |
| VP-LOGIN-01 | 正常ログイン  | 正常系        | 有効資格情報でセッション発行しダッシュボードに遷移する | §2.1, 図1  |        72 | Medium   | 利用頻度高、回帰リスク   | ユーザ有効         | 既存ユーザ: active=true | UI遷移/Set-Cookie/監査ログ | auth       |            |             |
| VP-LOGIN-02 | 入力境界    | 準正常系         | メール/パスワードの桁数・形式の境界で妥当性検証する  | §2.2, 表A  |        64 | Medium   | 入力不備は多発       | 未ログイン         | 最短/最長/空/全角混在       | UIバリデーション/ログ         | validation |            |             |
| VP-LOGIN-03 | 連続失敗ロック | 異常系      | 失敗5回で15分ロックし、期間中は拒否される      | §2.3      |        88 | High     | 認証回りは攻撃対象/苦情大 | 未ログイン         | 失敗回数=5             | UIエラー/監査ログ/レート制限     | security   |            |             |
| VP-LOGIN-04 | 監査ログ    | 非機能 | 成功/失敗/ロック操作が監査ログに完全記録される    | §4.1      |        84 | High     | 監査要件/不正調査     | 任意            |                    | ログ整合/改ざん検知           | audit      |            |             |

**所感**: 異常系/非機能 を含むが、セッション期限切れ再認証の記載が薄い。
...
| ViewpointID | Title   | TestCaseType      | Description              | SourceRef | RiskScore | RiskRank | RiskDetail  | Preconditions | DataNeeds | Observability | Tags     | Duplicates | Assumptions |
| ----------- | ------- | ------------- | ------------------------ | --------- | --------: | -------- | ----------- | ------------- | --------- | ------------- | -------- | ---------- | ----------- |
| VP-ORD-01   | 承認後のみ引当 | 準正常系         | ステータス=承認後にのみ在庫引当が可能      | §3.2      |        76 | High     | 誤引当で二重出荷リスク | 受注=承認         | SKU, qty  | DB状態/業務ログ     | workflow |            |             |
| VP-ORD-02   | 在庫不足のBO | 異常系      | 在庫不足時はバックオーダに遷移し通知される    | §3.3      |        90 | High     | 欠品は顧客影響大    | 受注=承認         | 在庫<需要     | 状態遷移/通知ログ     | stock    |            |             |
| VP-ORD-03   | 締日ロック   | 異常系      | 締処理中は出荷確定がブロックされる        | §5.1      |        80 | High     | 会計影響/不整合    | 締処理=実行中       | 売上日=月末    | UIエラー/監査ログ    | closing  |            |             |
| VP-ORD-04   | 伝票/監査   | 非機能 | 出荷確定で伝票発行・改ざん防止番号・監査項目記録 | §4.4      |        72 | Medium   | 監査/税務要件     | 出荷確定可能        |           | 伝票PDF/監査ログ    | audit    |            |             |
...
【要件】
1) 各観点に TestCaseType を `正常系 | 準正常系 | 異常系 | 非機能` で必ず付与。
...
7) 最後に 正常系/準正常系/異常系/非機能 の件数比と盲点を要約。
...
* [ ] 正常系/準正常系/異常系/非機能 の**比率が偏りすぎていない**。

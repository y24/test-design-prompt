...
* レビュー結果の最終アウトプット（必須要素）:
  ...
  * **採点（各100点満点）**:
    ...
    * 非機能の組み込み
  * **問題点の指摘**（根拠と参照情報つき）
  * **追加すべきテストケースの提案**（指摘内容に応じた具体的なテストケース案）
  * **次のアクション**（優先度・担当・粒度）
* 出力: Markdownファイル（2ファイル）。フォーマットおよび詳細テンプレートは §6 を参照。  
  出力先: `results` フォルダに出力する。
...
* 前処理（LLM側の自己チェック）:
  ...
  * **テスト観点抽出**: `test_viewpoint_identify_guide.md` に従って、元ドキュメントからテスト観点（正常系/準正常系/異常系、非機能）を体系的に抽出し、後続のレビューで使用する。抽出結果は **`test-viewpoint-list.md`** として出力する。
...
7. **追加テストケースの提案**: 指摘内容に応じて、追加すべきテストケースを具体的に提案する。
   * 非機能が不足している場合: 想定される非機能テストケース（パフォーマンス、セキュリティ、操作性、可用性、ログ/監査、アクセシビリティなど）を提案。
   * 異常系が不足している場合: 想定される異常系テストケース（想定外エラー、通信断、整合性崩壊、境界値エラーなど）を提案。
   * 準正常系が不足している場合: 想定される準正常系テストケース（想定内エラー、境界近傍、回復可能な分岐など）を提案。
   * 各提案には、テストケースID（推奨）、タイトル、前提条件、手順、期待結果、テストタイプ、関連する観点ID、根拠（元ドキュメントの参照箇所）を含める。
8. **出力整形**: `test_review_guide.md` §7 のテンプレートで Markdown を生成。参照根拠を**行番号/節番号**で示す。
...
* **追加テストケース提案の義務**: 指摘した欠落や不足に対しては、必ず具体的な追加テストケースを提案する。提案には以下を含める:
  * テストケースID（推奨形式: `TC-PROP-001` など）
  * タイトル（簡潔で明確な名称）
  * テストタイプ（正常系/準正常系/異常系/非機能）
  * 前提条件
  * 手順（ステップバイステップ）
  * 期待結果（観測可能な形で記述）
  * 関連する観点ID（`test-viewpoint-list.md`から参照）
  * 根拠（元ドキュメントの参照箇所、または提案理由）
...
* 列が不足する場合は、`Type`（正常系/準正常系/異常系/非機能）や `Tags` を補完提案。

...
* 出力は **JSON**（構造は`rules.md`に準拠。ここでは記述省略）と **Markdown** の二形式。
...
必ず以下を守る：
- 出力は JSON と Markdown の二形式（JSON構造は rules.md 準拠、ここではスキーマ説明不要）。
- シナリオは Given/When/Then を核に、目的・終了条件・観点トレーサビリティ(covers_vp)・観測手段(observability)を含める。
- 単機能断片の列挙は不可。ユーザーストーリー／業務フローの開始条件→分岐→終了条件で構成する。
- リスクは取り込んだ観点の risk_score を合成（maxベース＋頻度補正）してシナリオ risk_score を決定。
- 高リスク観点(score>=4)は必ずカバー。中低リスクは代表シナリオに編入してケース爆発を抑制。
- すべての表現は原文用語準拠。出典(source_refs)はシナリオにも残す。思考過程は出力しない。
```
...
# リスク合成ルール（簡約）
- scenario_risk_score = max(covers_vp.risk_score) を基本とし、主経路に登場する高頻度観点が多い場合は +1（上限5）。
...
4. **リスク合成**：取り込んだ観点の `risk_score` を max ベースで合成、主経路頻度で補正。
5. **最小本数化**：重複区間を統合、代表データで束ね、max_scenarios を満たすよう調整。
6. **可観測性の付与**：UIメッセージID・ログキー・DB更新指標を明記。
7. **検収**：`rules.md`のPhase 2 検収ルールを充足するまで自己修正。
...
* [ ] 各シナリオに **objective / actor / given / when / then / steps / covers_vp / risk_score / end_condition / observability / source_refs**
* [ ] すべての **risk>=4 観点**が少なくとも1本のシナリオでカバー
* [ ] 単機能の断片列挙になっていない（開始条件→分岐→終了条件で構成）
* [ ] トレーサビリティ欠落なし（covers_vpに観点が正しく紐付く）
* [ ] 用語は原文準拠、データ前提が再現可能

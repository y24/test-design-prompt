...
* ソース文書から、**正常系／準正常系／異常系**を横断する**テスト観点**を抽出する。
...
- 正常系 / 準正常系 / 異常系 を主機能ごと最低1件ずつ確保（rules.md §4 Phase 1検収ルール）。
...
|--------|--------------------------------|---------|------------|------|----------|----------------------------|
| VP-001 | 有効資格情報でのログイン成功   | ログイン | 正常系     | 3    | Med      | DOC-OPS-001 §3.2 p.12      |
| VP-002 | 連続失敗時のロック発動と解除   | ログイン | 準正常系      | 4    | High     | DOC-OPS-001 §3.2 p.13      |
| VP-003 | パスワードポリシー逸脱の拒否   | ログイン | 異常系   | 4    | High     | DOC-OPS-001 §3.2 p.12      |
```
...
| vp_id  | title                       | feature | test_class | risk | priority | source                     |
|--------|------------------------------|---------|------------|------|----------|----------------------------|
| VP-010 | 必須項目を満たす受注の登録   | 受注登録 | 正常系     | 3    | Med      | DOC-SRS-002 §5.1 p.44      |
| VP-012 | 在庫不足時の警告と保存許可   | 受注登録 | 準正常系      | 4    | High     | DOC-SRS-002 §5.1 p.46      |
| VP-013 | 単価ゼロや割引率超過の拒否   | 受注登録 | 異常系   | 4    | High     | DOC-SRS-002 §5.1 p.45      |
```
...
* [ ] 主機能ごとに **正常系/準正常系/異常系 >= 1**（`rules.md` §4 Phase 1検収ルール）
...
* **正常系 だけ量産** → 先に準正常系・異常の候補を列挙してから観点化。

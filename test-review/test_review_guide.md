# test_review_guide.md — テストケースレビュー用プロンプトガイド（LLM用）
...
1. 観点抽出結果（Markdown 表）: `ViewpointID, Title, TestType, SourceRef, RiskScore, ...`
...
1. **正規化**: CSV 列名/値の表記ゆれを吸収し、`Type` を `Normal/Quasi/Abnormal/NonFunctional` に正規化。ID 重複/飛び番/空行を検出。
...
3. **ギャップ分析**:
   ...
   * **偏り**（Normal 過多、Abnormal/NonFunctional 不足）
4. **品質評価**（各100点）:

   * **網羅性**: 観点カバー率、Normal/Quasi/Abnormal/NonFunctional 比率の妥当性、業務フローの分岐/例外の反映。
...
* **正常偏重**: Abnormal/Quasi が薄い。境界値/エラー遷移/通信断が抜けがち。
...
- 未カバー観点: <VP-IDs>
- 過剰/重複: <ケースIDクラスタ例>
- Type 比率: Normal <n> / Quasi <n> / Abnormal <n> / NonFunctional <n>
...
* 本ガイドは `rules.md` に準拠。スコア基準/ランク閾値はチーム合意でチューニング可。

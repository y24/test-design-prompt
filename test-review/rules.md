# rules.md — テストケースレビュー運用ルール（LLM用）

この文書は、LLM に「利用手順書/仕様書/要件定義書（以下、元ドキュメント）」と「作成済みテストケース（CSV）」を渡し、テストケースレビューを**均質化**するための大枠ルールを定める。詳細なプロンプトは別紙 `test_viewpoint_identify_guide.md` と `test_review_guide.md` を参照。
...
* 入力:
  ...
  2. テストケース CSV（列名例: `ID, Title, Steps, Expected, Type, Tags, Priority, Risk, Owner` など）
...
* 前処理（LLM側の自己チェック）:
  ...
  * 元ドキュメントから**テスト観点候補**（正常/準正常/異常、非機能）を粗く抽出し、後続の照合に使う（詳細は `test_viewpoint_identify_guide.md`）。
...
* デフォルト重み: すべて同一重み（各20%）。
* 重点レビュー観点が指定された場合、その項目の重みを**少しだけ**上げて再計算してもよい（例: 指定項目 30%、残り均等）。
* 総合評価ランク決定（例）:
...
2. **観点対照表の作成**: `test_viewpoint_identify_guide.md` に沿って抽出した観点リストと、CSV のケースを**1:多**でマッピング。
3. **ギャップ分析**: 未カバー観点、重複ケース、偏り（正常系過多など）を可視化。
4. **品質評価**: §3 の各基準で採点。リスクや優先度との整合も確認。
5. **改善提案**: ケース統合/分割、期待結果の可観測化、非機能の追加、データ駆動化など、**具体的**な次アクションを起票。
6. **出力整形**: §6 のテンプレートで Markdown を生成。参照根拠を**行番号/節番号**で示す。
...
- スコア
  ...
  - 再利用性: <0-100>
- 重点観点: <ユーザー指定があれば記載>
- 対象: <元ドキュメント名/版/日付> / <CSV名> / <範囲>

## 主要所見（根拠つき）
1. <所見1>  
   - 根拠: <元ドキュメントの節・ページ・行など>  
   - 該当ケース: <ID列挙>  
2. <所見2> ...
...
- 未カバー観点: <観点ID/名称>
- 過剰/重複: <ケースID>
- 偏り: <正常/準正常/異常の比率、非機能の不足など>

## リスク観点
- 高リスク領域: <領域名/理由>（推定影響度、障害費用、頻度など）
- 優先実施セット（例）: <ケースID一覧>
...
## 8. よくある欠陥パターン（レビューの観点）

* 期待結果が**UI文言**に過度依存／観測可能性が低い（ログ/DB/イベント基準がない）。
* 前提条件・テストデータの**準備手順**があいまい。
* 正常系偏重で、準正常/異常が薄い。例外や境界の視点が不足。
* 非機能（性能、セキュリティ、監査、アクセシビリティ）が**別紙任せ**でテストケースに織り込まれていない。
* ケース粒度が不均一（巨大なシナリオ vs. 細切れ単機能）。
* 重複ケースや、似た観点の**バリエーション乱立**。
* 固定値ハードコードで、環境差・文言差に弱い。

...
* 抽出済みの**テスト観点**（正常系／準正常系／異常系）を入力に、ユーザーストーリーや業務フローに沿った**業務シナリオ**へ束ねる。
...
- 正常系/準正常系/異常系 を可能な範囲で1本に混在させ、分岐点で準正常系・異常の回復やフォールバックを織り込む。
...
2. **観点マッピング**：主要経路の各ステージに `viewpoints` を割り当て、空白（正常系/準正常系/異常系）を検知。
3. **分岐の挿入**：準正常系（回復可能）と異常（想定外）を**支流として接続**し、戻りやフォールバックを明記。
...
- steps:
  1. 登録画面で必須入力→送信（正常）
  2. メール受信→認証リンクを開く（準正常系：メール遅延時の再送確認）
...
* VP-010 正常系：必須項目が満たされれば受注保存。
* VP-012 準正常系：在庫不足時は**警告表示の上、保存は可能**（出荷時に再検証）。
* VP-013 異常系：単価0や割引率>50%は保存拒否。
...
- steps:
  1. 必須項目を入力して受注保存（正常系: VP-010）
  2. 在庫不足の警告を確認し保存続行（準正常系: VP-012）
  3. 出荷予約時に再検証が走り、不足なら予約不可を確認（準正常系派生）
  4. 単価0や割引>50%のデータでは保存自体が拒否されることを別枝で確認（異常系: VP-013）
...
* VP-201 正常系：仕訳CSVのフォーマットに合致すれば取込成功。
* VP-202 準正常系：軽微な行エラーはスキップし、サマリに件数・原因を記録。
* VP-203 異常系：承認権限がないユーザーの承認操作は拒否。
* VP-204 異常系：締め処理中に新規仕訳が入るとロールバックし再実行指示。
...
- steps:
  1. CSV取込（正常系: VP-201）→サマリ出力を確認
  2. スキップ発生（準正常系: VP-202）→原因集計と件数を確認
  3. 無権限ユーザーの承認試行が拒否されること（異常系: VP-203）
  4. 締め処理中の新規仕訳投入でロールバック→再実行指示を確認（異常系: VP-204）
...
* **症状**：観点をそのまま1対1で“テストケース化”
  ...
  * **回避**：観点は束ねて物語線を作る。分岐を内包し、準正常系→回復、異常→フォールバックを組み込む。
* **症状**：高リスク観点がシナリオ外
...
```yaml
VIEWPOINTS_BLOCK: |
  [ { "vp_id":"VP-010","feature":"受注登録","test_class":"正常系","risk_score":3 }, ... ]
STORY_CONTEXT_BLOCK: |
  actors: ["一般ユーザー","管理者"]
